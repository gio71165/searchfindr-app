    ...
    <div className="min-h-full bg-slate-900 px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <button
          ...
          className="btn-primary inline-flex items-center gap-2 shadow-sm hover:shadow-md"
        >
      ...
      <div className="mb-6 space-y-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-slate-300 placeholder-slate-500 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
          />
        </div>
        ...
        <div className="flex flex-wrap gap-4 items-center">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="px-3 py-1.5 bg-slate-800 border border-slate-700 rounded-lg text-slate-300 text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
            >
      ...
      {filteredDeals.length === 0 ? (
        <div className="bg-slate-800 rounded-xl border border-slate-700 p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-slate-50 mb-2">No deals found</h3>
          ...
          {filteredDeals.map((deal) => {
            ...
            return (
              ...
              >
                ...
                <div className="p-4">
                  <div className="flex items-start gap-4">
                    ...
                    <button
                      ...
                      className="flex-shrink-0 mt-1 text-slate-400 hover:text-slate-300"
                    >
                      {isExpanded ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Link
                            href={`/deals/${deal.id}`}
                            className="btn-primary btn-sm shadow-sm hover:shadow-md"
                            onClick={(e) => e.stopPropagation()}
                          >
                            View Deal
                          </Link>
                ...
                {isExpanded && hasDocuments && (
                  <div className="border-t border-slate-700 divide-y divide-slate-700">
                    {deal.documents.map((doc) => (
                      <div
                        key={doc.id}
                        className="px-4 py-3 flex items-center justify-between hover:bg-slate-700/50 transition-colors"
                      >
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          <span className="text-lg">{DOCUMENT_TYPE_ICONS[doc.document_type || 'other']}</span>
                          <div className="flex-1 min-w-0">
                            <p className="font-medium text-slate-50 truncate">{doc.filename}</p>
                        ...
                        <div className="flex items-center gap-2 ml-4">
                          <Link
                            ...
                            className="px-3 py-1.5 text-sm text-slate-400 hover:text-emerald-400 hover:bg-emerald-500/10 rounded-lg transition-colors"
                            onClick={(e) => e.stopPropagation()}
                          >
                            View
                          </Link>
                ...
                {isExpanded && !hasDocuments && (
                  <div className="border-t border-slate-700 px-4 py-6 text-center">
                    <FileText className="h-8 w-8 text-slate-500 mx-auto mb-2" />
                    <p className="text-sm text-slate-400 mb-4">No documents attached to this deal</p>
                    <Link
                      href={`/deals/${deal.id}#documents`}
                      className="btn-primary inline-flex items-center gap-2 shadow-sm hover:shadow-md"
                      onClick={(e) => e.stopPropagation()}
