...
function clipText(s: any, maxChars = 9000) {
...
export async function chatForDeal(args: DealChatArgs): Promise<DealChatResult> {
  ...
  const { userMessage, dealContext, history } = args;
...
`.trim();
  ...
  // Build a single “context” blob from server-truth fields
  const contextBlock = `
...
`.trim();
  ...
  // Keep only the last 10 messages from history (and only allowed roles)
  const cleanedHistory = (history ?? [])
    .slice(-10)
    .filter((m) => m && (m.role === "user" || m.role === "assistant" || m.role === "system"))
    .map((m) => ({ role: m.role as ChatRole, content: String(m.content ?? "").slice(0, 4000) }));

  // Use Chat Completions for simplicity/stability in server routes
  ...
  const answer = resp.choices?.[0]?.message?.content?.trim() || "I couldn't generate an answer.";
  ...
  // Sources used = which fields we passed (helps later)
  const sources_used = [
